...
- Repo `~/Development/LisaEmu`; branch `m2-floppy-boot` from current main (post-M1c, `ce7a2b1`); commit per task, never amend; TDD for device code; investigative tasks produce evidence docs.
- M1c exists: LisaShell (EmulationController: emulation thread, mailbox `Command` enum — the extension seam for disk commands; `Shared` for cross-thread config; `debugSync` test seam) and LisaApp (Xcode project via `xcodegen generate --spec LisaApp/project.yml`; xcodebuild test target LisaAppTests). App-facing work lands in Task 7. `EmulationController.reset()` currently RECREATES the Machine — inserted-media state must survive reset (see Task 2/Task 7 interfaces).
...
- M1c interplay: with Machine-level warm reset available, evaluate switching `EmulationController.reset()` from recreate-Machine to `machine.reset()` on the emulation thread (simpler AND preserves inserted media); if recreate is retained for any reason, media config MUST move into `Shared` and be reapplied on recreate. Either way: a test proving an inserted disk survives controller reset.
...
- [ ] **Step 3:** docs/m2-demo.md (reproduce commands + screenshot path outside repo, ~/Development/LisaEmu-artifacts/m2-loader-screen.png if the loader draws anything; else the lisadbg status transcript). Full regression: suites green both ways + full TomHarte release run 807147/0/192913 (re-extract the gzipped vectors first: `find Vendor/ProcessorTests/680x0/68000/v1 -name '*.json.gz' -exec gunzip -kf {} +`; delete the .json files after, keep .gz — established disk-hygiene procedure). Update memory-worthy facts in rom-trace-notes final section. Commit.

---

### Task 7: App integration — insert, boot, and watch it live

**Files:**
- Modify: `Sources/LisaShell/EmulationController.swift` (+`insertFloppy(url:)`/`ejectFloppy()` mailbox commands; disk state surfaced in `EmuStatus`), `LisaApp/Sources/LisaApp.swift` (File → Insert Disk… ⌘I via NSOpenPanel filtered to dc42; Eject), `LisaApp/Sources/ScreenView.swift`/`AppModel.swift` (drag-and-drop of a .dc42 onto the window → insert; status strip gains a disk-activity indicator), `docs/m2-demo.md`, `LisaApp/project.yml` only if needed
- Test: `Tests/LisaShellTests/EmulationControllerTests.swift` additions; `LisaApp/Tests/LisaAppTests` additions for pure logic

**Interfaces:**
- `EmulationController.insertFloppy(url: URL)` posts a mailbox command; the emulation thread loads the DC42 (load errors published via a status/error surface, never a crash) and calls the FloppyController seam. `ejectFloppy()` symmetric. Media survives `reset()` per Task 2's interface (test proves it). `EmuStatus` gains `diskInserted: Bool` and `diskActivity: Bool` (activity = controller processed a command in the last status interval; simple and documented).
- Conscious scheduling of the M1c-deferred spec §4 items: the status-strip **disk-activity indicator lands here**; "power on/off via COPS" is explicitly DEFERRED AGAIN to M3 (record in the completion notes: `COPS.powerCommandLog` exists; a Power menu belongs with the soft-power/Widget work) — deliberate and recorded, not lost.
- ROM-gated LisaShellTests integration: `insertFloppy(OS31_Install_1.dc42)` → the proven scripted STARTUP FROM click sequence (M1c Task 4's coordinates/mechanism) → assert Task 6's loader-execution markers are reached through the APP-facing path. The M2 demo, made automatic.
- Manual checkpoint: add a `--insert-disk <path>` debug launch argument (documented alongside `--auto-screenshot`); capture the loader screen from the real app to ~/Development/LisaEmu-artifacts/m2-live-loader.png (never committed).

- [ ] **Step 1:** Failing controller tests (insert/eject/survives-reset/status fields). **Step 2:** Implement controller + app UI + drag-drop + indicator. **Step 3:** ROM-gated integration test green; xcodebuild test green; manual checkpoint captured. **Step 4:** docs/m2-demo.md gains the in-window flow ("Insert Disk 1, click STARTUP FROM, watch the Office System loader"). Commit(s).
